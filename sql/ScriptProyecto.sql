...
/*  Created On : 19-Aug-2020 6:31:01 PM 				*/
/*  DBMS       : Oracle 						*/
/* ---------------------------------------------------- */
...
CREATE TABLE    Banco 
(
	  NUMERO_TARJETA  NUMBER(38) NOT NULL,
	  CVV  NUMBER(3) NOT NULL,
	  FECHA_EXP  DATE NOT NULL,
	  FONDOS  NUMBER(30) NOT NULL,
	  ID_CEDULA  NUMBER(20) NOT NULL,
...
(
	  ...
	  TELEFONO_DESTINO  NUMBER(15) NOT NULL,    -- Numero de teléfono de quien resive
	  NOMBRE_DESTINO  VARCHAR2(50) NOT NULL,    -- Mombre de la persona que recibe
	  ...
	  CIUDAD_DESTINO  VARCHAR2(15) NOT NULL
)
;
...
(
	  ...
	  ESTADO_PEDIDO  VARCHAR2(2) NOT NULL,    -- Fase actual del pedido 'CA' - carrito, 'PP' -pendiente pago, 'PA' - pagado, 'RE' - rechazado, 'CV' - carrito vencido
...
(
	  ...
	  FECH_EXP  DATE NOT NULL,    -- Fecha de expiracion impreso en la tarjeta
...
COMMENT ON TABLE    Banco  IS 'Tabla que simula la existencia de un banco'
;
...
COMMENT ON COLUMN    Pedido .  ESTADO_PEDIDO  IS 'Fase actual del pedido ''CA'' - carrito, ''PP'' -pendiente pago, ''PA'' - pagado, ''RE'' - rechazado, ''CV'' - carrito vencido'
;
...
ALTER TABLE    Envio  
 ADD CONSTRAINT   CK_METODO_PAGO  CHECK (METODO_PAGO IN ('C','P','D'))
;
...
ALTER TABLE    Pedido  
 ADD CONSTRAINT   CK_ESTADO_PEDIDO  CHECK (ESTADO_PEDIDO IN ('CA','PP','PA','RE','CV'))
;
...
ALTER TABLE    Direccion  
 ADD CONSTRAINT   FK_DIRECCION_ENVIO 
	FOREIGN KEY (  ID_CEDULA ,  TIPO_ID ) REFERENCES   ()
;
